...
if (isset($_SESSION["Admin"])) {
    if (isset($_GET["action"]) && isset($_GET["id"])) {
        if ($_GET["action"] == "delete") {
            ...
            $list_noti= returnCURL($array_send_manage, SITE_NAME_MANAGE.'/controller/admin/tour_list_dichvu.php');
            if($list_noti){
...
//                if (isset($_GET["check_" . $tour_list_dichvu->id])) tour_list_dichvu_delete($tour_list_dichvu);
    ...
    if (isset($_POST["name"]) && isset($_POST["type"]) && isset($_POST["price"]) && isset($_POST["number"]) && isset($_POST["note"])) {
        ...
        if($array_send_manage['tour_id']){
            $data_detail=tour_getById($array_send_manage['tour_id']);
            if($data_detail){
                $array_send_manage['tour_id']=$data_detail[0]->code_az_mix;
            }else{
                header('Location: ' . SITE_NAME . '/controller/admin/tour_list_dichvu.php' . $link);
            }
        }else{
