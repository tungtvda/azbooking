...
{
   return khachsan_room_price_Get("SELECT khachsan_room_price.id, khachsan.name as danhmuc_id, khachsan_room_price.name, khachsan_room_price.price FROM  khachsan_room_price, khachsan where khachsan.id=khachsan_room_price.danhmuc_id  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function khachsan_room_price_insert($obj)
{
    return exe_query("insert into khachsan_room_price (danhmuc_id,name,price) values ('$obj->danhmuc_id','$obj->name','$obj->price')",'khachsan_room_price');
}
//
function khachsan_room_price_update($obj)
{
    return exe_query("update khachsan_room_price set danhmuc_id='$obj->danhmuc_id',name='$obj->name',price='$obj->price' where id=$obj->id",'khachsan_room_price');
}
//
