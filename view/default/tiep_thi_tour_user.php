...
function show_tiepthi_tour_user($data = array())
{
    ...
    $asign['mess_null'] = '';
    $dem=1;
    $asign['tour_id_show_popup'] = '';
    $listDanhsach='';
    if(isset( $data['detailTour'])) {
        $asign['tour_id_show_popup'] = "<input hidden id='tour_id_show_popup' value='".$data['detailTour']['id']."'><input hidden id='tour_name_show_popup' value='".$data['detailTour']['name_tour']."'>";
        $listDanhsach = returnHtmlListTourUser($data['detailTour'],$dem);
        $dem = 2;
    }
...
function returnHtmlListTourUser($row,$dem){
    switch($row['status']){
        case '1':
    ...
    $tour.='<input hidden value="'.$row['note_confirm'].'" id="note_confirm_hidden_'.$row['id'].'">';

   return $listDanhsach = '<tr id="tr-tour-'.$row['id'].'">
